...
#define POS_DIGMODE_IND_Y              290

// S meter position
#define POS_SM_IND_X                58
#define POS_SM_IND_Y                0

// Supply Voltage indicator
//#define POS_PWRN_IND_X              0
//#define POS_PWRN_IND_Y              290
#define POS_PWR_WIDTH               50

//#define POS_PWR_IND_X               330
...
#define POS_LOAD_X                  0
#define POS_LOAD_Y                  305

//???
#define POS_SYNCAM_X                80
#define POS_SYNCAM_Y                POS_DIGMODE_IND_Y
//-----------------------------------------------------------------------------------------------------------------------------------------
#else
// Frequency display control
...
void UiDriver_HandleTouchScreen()
{
    ...
    {
        if(UiDriver_CheckTouchCoordinates(sX2T(POS_SM_IND_X),sX2T(POS_SM_IND_X+200),sY2T(POS_SM_IND_Y+58),sY2T(POS_SM_IND_Y))) // S-Meter
        {
...
void UiDriver_FButtonLabel(uint8_t button_num, const char* label, uint32_t label_color)
{
    ...
    UiLcdHy28_PrintTextCentered(POS_BOTTOM_BAR_F1_X + (button_num - 1)*(POS_BOTTOM_BAR_BUTTON_W+2), POS_BOTTOM_BAR_F1_Y, 56, label,
                                label_color, Black, 0);
...
#define ENC_COL_W 28
#define ENC_ROW_H (15)
#define ENC_COLUMN_SPACING 6      //odstep miedzy boxami
                            ...
                            const char temp[5], uint32_t color)
{
    ...
    UiLcdHy28_DrawEmptyRect(POS_ENCODER_IND_X + ENC_COL_W * (column*4 + row*2)+column*ENC_COLUMN_SPACING, POS_ENCODER_IND_Y, ENC_ROW_H - 2, ENC_COL_W*2 - 2, brdr_color);
    UiLcdHy28_PrintTextCentered((POS_ENCODER_IND_X + 1 + ENC_COL_W * (column*4 + row*2)+column*ENC_COLUMN_SPACING), (POS_ENCODER_IND_Y + 1 ),ENC_COL_W - 3, label,
                        label_color, bg_color, 0);
    UiLcdHy28_PrintTextRight((POS_ENCODER_IND_X + ENC_COL_W - 4 + ENC_COL_W * (column*4 + row*2+1)+column*ENC_COLUMN_SPACING), (POS_ENCODER_IND_Y + 1), temp,
                             color, Black, 0);
}
//------------------------------------------------------------------------------------------------------------------------------------------------
#else
...
void UiDriver_UpdateDisplayAfterParamChange()
{
    ...
    UiSpectrum_InitSpectrumDisplay();   //sp9bsl  bo brak ustawienia wlasciwych parametrow podswietlenia widma
...
static void UiDriver_DisplayMemoryLabel()
{
     if (ts.band < MAX_BAND_NUM)
    {
        ...
        UiLcdHy28_PrintText(80+(SMALL_FONT_WIDTH * 11)+4,  290,txt,col,Black,0);
    }
 }
...
static void UiDriver_CreateVoltageDisplay() {
    // Create voltage
    UiLcdHy28_PrintTextCentered (POS_PWR_IND_X,POS_PWR_IND_Y,POS_PWR_WIDTH,   "--.- V",  COL_PWR_IND,Black,0);
}
...
//    UiLcdHy28_PrintText(10,140,txt_buf,clr_fg,clr_bg,0);
            ...
            UiLcdHy28_LcdClear(clr_bg);
            clr_fg = White;
            UiLcdHy28_PrintText(10,290,"+",clr_fg,clr_bg,1);
...
//    UiLcdHy28_PrintText(10,140,txt_buf,clr_fg,clr_bg,0);
            ...
            UiLcdHy28_LcdClear(clr_bg);
            clr_fg = White;
            UiLcdHy28_PrintText(10,150,"              +",clr_fg,clr_bg,1);
    ...
    if(UiDriver_TimerIsExpired(SCTimer_MAIN,now,1))            // bail out if it is not time to do this task
    {
        ...
        case STATE_LO_TEMPERATURE:
            if (UiDriver_TimerExpireAndRewind(SCTimer_LODRIFT,now,64))
            {
                ...
                snprintf(str,20,"%3u%%",(unsigned int)load);
                UiLcdHy28_PrintText(POS_LOAD_X,POS_LOAD_Y,str,White,Black,0);
#endif
            }
        ...
        case STATE_DISPLAY_SAM_CARRIER:
            if (UiDriver_TimerExpireAndRewind(SCTimer_RTC,now,25))
            {
                ...
                static uint8_t Old_agc_wdsp=0xff;
                if(Old_agc_wdsp!=ts.agc_wdsp)       //detection of display change (SP9BSL)
                {
                    Old_agc_wdsp=ts.agc_wdsp;
                    const char* txt = "   ";
                    ...
                    UiLcdHy28_PrintTextCentered(POS_SYNCAM_X,POS_SYNCAM_Y,POS_DEMOD_MODE_MASK_W-6,txt,AGC_color2,AGC_color,0);
                }
            }
            break;
        default:
