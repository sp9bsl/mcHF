...
uint16_t ui_spectrum_line[POS_SPECTRUM_IND_H];
uint16_t ui_spectrum_lineFLTR[POS_SPECTRUM_IND_H];
uint16_t ui_spectrum_grid[POS_SPECTRUM_IND_H];
uint16_t ui_spectrum_gridV[POS_SPECTRUM_IND_H];
uint16_t ui_spectrum_gridFLTR[POS_SPECTRUM_IND_H];

void UiSpectrum_CalcDisplayFilterBW(void);
static void     UiSpectrum_FrequencyBarText();
//static void		UiSpectrum_CalculateDBm();
static void UiSpectrum_CalculateDBm(float* samplesData, float* MagData);
...
void UiSpectrum_CreateDrawArea()
{
    ...
    //
    UiLcdHy28_PrintText(POS_SPECTRUM_IND_X, POS_SPECTRUM_IND_Y + POS_SPECTRUM_FREQ_BAR_Y, "                                 ", Black, Black, 0);

    // Frequency bar separator
    UiLcdHy28_DrawHorizLineWithGrad(POS_SPECTRUM_IND_X,(POS_SPECTRUM_IND_Y + POS_SPECTRUM_FREQ_BAR_Y + POS_SPECTRUM_FREQ_BAR_H-1),POS_SPECTRUM_IND_W,COL_SPECTRUM_GRAD);
    ...
    UiLcdHy28_DrawStraightLine (POS_SPECTRUM_IND_X + POS_SPECTRUM_VGridSpacing*ts.c_line + 1, (POS_SPECTRUM_IND_Y +POS_SPECTRUM_FREQ_BAR_H), (POS_SPECTRUM_IND_H - POS_SPECTRUM_FREQ_BAR_H), LCD_DIR_VERTICAL, ts.scope_centre_grid_colour_active);


    UiSpectrum_CalcDisplayFilterBW();   //calculate bandwidth highlight
    ...
    if(!ts.spectrum_size)		//don't draw text bar when size is BIG
    {
        ...
        for(int i = 0; i < 16; i++)
        {
            UiLcdHy28_DrawHorizLineWithGrad(POS_SPECTRUM_IND_X,(POS_SPECTRUM_IND_Y +i),POS_SPECTRUM_IND_W,COL_SPECTRUM_GRAD);
        }
    ...
    clr=(clr >> 8) | (clr << 8);
    int i;
    uint16_t clrHGrid=UI_SPECTRUM_HIGHLIGHTGridColor;
    uint16_t clrHBack=UI_SPECTRUM_HIGHLIGHTGridBackColor;
    clrHBack=(clrHBack >> 8) | (clrHBack << 8);
    clrHGrid=(clrHGrid >> 8) | (clrHGrid << 8);
    uint16_t clrHL=UI_SPECTRUM_HIGHLIGHTForeColor;
    clrHL=(clrHL >> 8) | (clrHL << 8);
    for(i=0;i<POS_SPECTRUM_IND_H-POS_SPECTRUM_FREQ_BAR_H;i++)
    {
        ui_spectrum_grid[i]=0;
        ui_spectrum_gridFLTR[i]=clrHBack;
        ui_spectrum_line[i]=clr;
        ui_spectrum_lineFLTR[i]=clrHL;
    }
    ...
    { // if spectrum display light disabled, bail out here!


        uint32_t grid_clr;

        grid_clr=ts.scope_grid_colour_active;

        grid_clr=(grid_clr >> 8) | (grid_clr << 8);

        for(i=0;i<POS_SPECTRUM_IND_H-POS_SPECTRUM_FREQ_BAR_H;i++)
        {
            ui_spectrum_gridV[i]=grid_clr;
        }

        // Horizontal grid lines
        uint16_t HGridLinesCount, y_subb;
        if(ts.spectrum_size)		//set range big/normal
        {
            HGridLinesCount = SpectrumGridY;    //spectrum size=big
            y_subb = 0;
        }
        else
        {
            HGridLinesCount = SpectrumGridY-2;  //spectrum size=normal
            y_subb = POS_SPECTRUM_INFO_BAR_H;
        }



        int SpectrumSizeY=POS_SPECTRUM_IND_H - POS_SPECTRUM_FREQ_BAR_H -y_subb;

        uint16_t tempPosY;
        for(i = 1; i < HGridLinesCount; i++)
        {
            // Save y position for repaint
            tempPosY=POS_SPECTRUM_IND_Y + POS_SPECTRUM_FREQ_BAR_H +y_subb + i*SpectrumSizeY/(HGridLinesCount);
            sd.horz_grid_id[i-1] = tempPosY;
        ...
        for(i = 0; i <(HGridLinesCount-1); i++)
        {
            tempPosY=sd.horz_grid_id[i]-POS_SPECTRUM_IND_Y-POS_SPECTRUM_FREQ_BAR_H - y_subb;
            ui_spectrum_grid[tempPosY]=grid_clr;
            ui_spectrum_gridFLTR[tempPosY]=clrHGrid;
        }

        for(i=sd.Pos_FilterStart;i<=sd.Pos_FilterEnd;i++)       //drawing background highlight
        {
            UiLcdHy28_OpenBulkWrite(i, 1, POS_SPECTRUM_IND_Y + y_subb + POS_SPECTRUM_FREQ_BAR_H, SpectrumSizeY);
            UiLcdHy28_SpiDmaStart((uint8_t*)ui_spectrum_gridFLTR,SpectrumSizeY*2);
        }
...
//        ts.c_line = UiSpectrum_GetGridCenterLine(4);
        ...
        for(int i = 1; i < SpectrumGridX; i++)
        {
            ...
            sd.vert_grid_id[i - 1] = (POS_SPECTRUM_IND_X + POS_SPECTRUM_VGridSpacing*i + 1);
            //sd.vert_grid_id[i - 1] = (POS_SPECTRUM_IND_X + 32*i + 1);

            // Draw

            if((sd.vert_grid_id[i - 1]>sd.Pos_FilterStart) && (sd.vert_grid_id[i - 1]<sd.Pos_FilterEnd))
            {
                if (i != ts.c_line)
                    clr=UI_SPECTRUM_HIGHLIGHTGridColor;
            }

            UiLcdHy28_DrawStraightLine(	sd.vert_grid_id[i - 1],
                    (POS_SPECTRUM_IND_Y + POS_SPECTRUM_FREQ_BAR_H +y_subb),
                    (POS_SPECTRUM_IND_H - POS_SPECTRUM_FREQ_BAR_H -y_subb),    //SpectrumSizeY
                    LCD_DIR_VERTICAL,
                    clr);
        ...
        if (((ts.flags1 & FLAGS1_WFALL_SCOPE_TOGGLE) && (!ts.waterfall_speed)) || (!(ts.flags1 & FLAGS1_WFALL_SCOPE_TOGGLE) && (!ts.scope_speed)))
        {
            ...
            UiLcdHy28_PrintText(			(POS_SPECTRUM_IND_X + POS_SPECTRUM_IND_W/2),
                    (POS_SPECTRUM_IND_Y + POS_SPECTRUM_FREQ_BAR_H),
                    "   DISABLED   ",
                    Grey,
                    RGB((COL_SPECTRUM_GRAD*2),(COL_SPECTRUM_GRAD*2),(COL_SPECTRUM_GRAD*2)),0);
        }
...
{
    UiLcdHy28_DrawFullRect(POS_SPECTRUM_IND_X, (POS_SPECTRUM_IND_Y), POS_SPECTRUM_IND_H+4+sd.wfall_height, POS_SPECTRUM_IND_W, Black);	// Clear screen under spectrum scope by drawing a single, black block (faster with SPI!)
}
...
void    UiSpectrum_DrawSpectrum(q15_t *fft_old, q15_t *fft_new, const ushort color_old, const ushort color_new, const ushort shift)
{
    ...
    {
        spec_height = spec_height - POS_SPECTRUM_INFO_BAR_H;
        spec_start_y = spec_start_y + POS_SPECTRUM_INFO_BAR_H;
    }
    ...
    if(shift)
    {
    	sh = (SPEC_BUFF_LEN/2) + 1;   // Shift to fill gap in center
    }
    else
    {
    ...
    {
        idx = 0;
        for(x = (SPECTRUM_START_X + sh); x < (POS_SPECTRUM_IND_X + SPEC_BUFF_LEN/2 + sh); x++)
        {
            if(x>=SPECTRUM_WIDTH)
                break;
            ...
            if (!(ts.flags1 & FLAGS1_SCOPE_LIGHT_ENABLE))
            {
                ...
                if((x>=sd.Pos_FilterStart) && (x<=sd.Pos_FilterEnd))
                    UiLcdHy28_DrawStraightLine(x,y1_new,y_new,LCD_DIR_VERTICAL,ui_spectrum_lineFLTR[0]);
                else
                    UiLcdHy28_DrawStraightLine(x,y1_new,y_new,LCD_DIR_VERTICAL,color_new);


                //UiLcdHy28_OpenBulkWrite(x, 1, y1_new, y_new);
                //UiLcdHy28_SpiDmaStart((uint8_t*)ui_spectrum_line,y_new);
            }
        }
        sd.first_run--;
    }
    else
    {
        ...
        idx = 0;
        int VgridNum=0; //indeks wskaznika numeru linii siatki V
        int DrawGridV=0;

        for(i=0;i<SpectrumGridX;i++)        //wyszukanie pierwszej wspolrzednej siatki
        {
            if((SPECTRUM_START_X + sh)>sd.vert_grid_id[VgridNum] && (VgridNum<SpectrumGridX))
                VgridNum++;
        }
        ...
        {
            if(x>=SPECTRUM_WIDTH)
                break;  //koniec bo przekroczy szerokosc ekranu

            if (ts.flags1 & FLAGS1_SCOPE_LIGHT_ENABLE)

            {
                if ((idx > 1) && (idx < (SPEC_BUFF_LEN/2-2))) //
                {
            ...
            else if (x == SPECTRUM_START_X + (SPEC_BUFF_LEN/2) + 1) // special case of first line of right part of spectrum
            {
            ...
            if ((ts.flags1 & FLAGS1_SCOPE_LIGHT_ENABLE) && x != (POS_SPECTRUM_IND_X + POS_SPECTRUM_VGridSpacing*ts.c_line + 1))
            {
                ...
                { // plot line upwards
                    ...
                    UiLcdHy28_OpenBulkWrite(x, 1, y1_old_minus + 1, y1_old - y1_old_minus);
                    UiLcdHy28_SpiDmaStart((uint8_t*)ui_spectrum_grid,(y1_old - y1_old_minus)*2);
                }
                ...
                { // plot line downwards
                    ...
                    UiLcdHy28_OpenBulkWrite(x, 1, y1_old, y1_old_minus-y1_old);
                    UiLcdHy28_SpiDmaStart((uint8_t*)ui_spectrum_grid,(y1_old_minus-y1_old)*2);

                }
                else
                {
                    //UiLcdHy28_DrawStraightLine(x,y1_old,1,LCD_DIR_VERTICAL,color_old);
                    UiLcdHy28_OpenBulkWrite(x, 1, y1_old, 1);
                    UiLcdHy28_SpiDmaStart((uint8_t*)ui_spectrum_grid,2);
                ...
                { // plot line upwards
                    ...
                    UiLcdHy28_OpenBulkWrite(x, 1, y1_new_minus + 1, y1_new - y1_new_minus);
                    UiLcdHy28_SpiDmaStart((uint8_t*)ui_spectrum_line,(y1_new - y1_new_minus)*2);
                ...
                { // plot line downwards
                    ...
                    UiLcdHy28_OpenBulkWrite(x, 1, y1_new, y1_new_minus - y1_new);
                    UiLcdHy28_SpiDmaStart((uint8_t*)ui_spectrum_line,(y1_new_minus - y1_new)*2);
                }
                else
                {
                    //UiLcdHy28_DrawStraightLine(x,y1_new,1,LCD_DIR_VERTICAL,color_new);
                    UiLcdHy28_OpenBulkWrite(x, 1, y1_new, 1);
                    UiLcdHy28_SpiDmaStart((uint8_t*)ui_spectrum_line,2);
            ...
            if (!(ts.flags1 & FLAGS1_SCOPE_LIGHT_ENABLE))
            {
                if(x>sd.vert_grid_id[VgridNum] && (VgridNum<SpectrumGridX))
                    VgridNum++;
                ...
                if(y_old <= y_new)
                {
                    ...
                    if (y_old != y_new)
                    {
                        ...
                        sendSize=y_new-y_old;
                        sendSize2=sendSize*2;
                        if((x>=sd.Pos_FilterStart) && (x<sd.Pos_FilterEnd))
                        {
                            UiLcdHy28_OpenBulkWrite(x, 1, y1_new, sendSize);
                            UiLcdHy28_SpiDmaStart((uint8_t*)ui_spectrum_lineFLTR,sendSize2);
                        }
                        else
                        {
                            UiLcdHy28_OpenBulkWrite(x, 1, y1_new, sendSize);
                            UiLcdHy28_SpiDmaStart((uint8_t*)ui_spectrum_line,sendSize2);
                        }
                ...
                {
                    sendSize=y_old-y_new;
                    sendSize2=sendSize*2;
                    startPos=spec_height-y_old-1;

                    if(x==sd.vert_grid_id[VgridNum])
                    {
                        //if(VgridNum==(x_center_line))
                        if(VgridNum==(ts.c_line-1))
                            clr=ts.scope_centre_grid_colour_active;
                        else
                            clr=ts.scope_grid_colour_active;


                        clr=(clr >> 8) | (clr << 8); //konwersja koloru

                        for(i=0;i<y_old-y_new;i++)
                            ui_spectrum_gridV[i]=clr;


                        UiLcdHy28_OpenBulkWrite(x, 1, y1_old, sendSize);
                        UiLcdHy28_SpiDmaStart((uint8_t*)(&ui_spectrum_gridV[0]),sendSize2);
                    ...
                    else
                    {
                        ...
                        {
                            UiLcdHy28_OpenBulkWrite(x, 1, y1_old, sendSize);
                            UiLcdHy28_SpiDmaStart((uint8_t*)(&ui_spectrum_gridFLTR[startPos]),sendSize2);
                        }
                        else
                        {
                            UiLcdHy28_OpenBulkWrite(x, 1, y1_old, sendSize);
                            UiLcdHy28_SpiDmaStart((uint8_t*)(&ui_spectrum_grid[startPos]),sendSize2);
                        }
                    }
/*
...
static void UiSpectrum_InitSpectrumDisplayData()
{
    ...
    */
    ...
    sd.db_scale*=(((float)POS_SPECTRUM_IND_H - 15.0)/5.0)/16.0;
/*
...
*/
     sd.wfall_height = WATERFALL_HEIGHT;
     if(!(ts.demod_mode_disable&DEMOD_DIGI_DISABLE))
         sd.wfall_height -=16;                      //if DV mode, leave some space for demodulation data

     sd.wfall_ystart = WATERFALL_START_Y;
     //sd.wfall_size = WATERFALL_HEIGHT + WFALL_MEDIUM_ADDITIONAL;
     sd.wfall_size = WaterfallDataLines;
    ...
    sd.enabled		= 1;
    sd.first_run 	= 2;
    sd.CollectSamplesEnable=1;
}

void UiSpectrum_ClearWaterfallData()
{
    for(int i = 0; i < (WaterfallDataLines); i++)   // clear old wf lines if changing magnify
    {
...
void UiSpectrum_RedrawScopeDisplay()
{
    ...
    if(ts.spectrum_scheduler == 0 && (ts.scope_speed > 0))	// is it time to update the scan, or is this scope to be disabled?
    {
        ...
        case 2:
        {
            ...
            arm_cfft_f32(&arm_cfft_sR_f32_len512, sd.FFT_Samples, 0, 1);    // Do complex FFT with new lib (faster! sexier! more accurate!?)
            ...
            if(sd.wfal_state==0)
            {
                arm_copy_f32(sd.FFT_MagData, sd.FFT_MagDataWF,SPEC_BUFF_LEN);   //kopiowanie danych fft dla funkcji pokazywania widma
                sd.wfal_state=3;
            }
            //
        ...
        case 3:
        {
            ...
            UiSpectrum_CalculateDBm(sd.FFT_Samples,sd.FFT_MagData);
        ...
        case 4:
        {
            ...
            for(i = 0; i < (SPEC_BUFF_LEN); i++)
            {
                ...
                if(sig>= spec_height) sig=spec_height - 1;   // is there an illegal height value?
        ...
        case 5:
        {
            ...
            //sd.state++;
            //sd.wfal_state =3;   //enable waterfall state machine
            //sd.wfal_state =0;   //enable waterfall state machine
            break;
        }
        case 6: //oczekiwanie na zakonczenie wyswietlania waterfall
            break;
        default:
            sd.state = 0;
            sd.CollectSamplesEnable=1;
...
//
uint16_t waterfall_pixel_buf[2][SPECTRUM_WIDTH];
uint8_t waterfall_pixel_buf_idx=0;
uint16_t debb;
void UiSpectrum_RedrawWaterfall()
{
    ulong i;

    if((ts.waterfall_scheduler == 0 ) && (ts.waterfall_speed > 0))	// is it time to update the scan, or is this scope to be disabled?
    {
        ts.waterfall_scheduler = (ts.waterfall_speed-1)*2;

        // Process implemented as state machine
        switch(sd.wfal_state)
        {
        //
        // Apply gain to collected IQ samples and then do FFT
        //
        ...
        case 2:		// Do FFT and calculate complex magnitude
        {
            ...
            arm_cmplx_mag_f32( sd.FFT_SamplesWF, sd.FFT_MagDataWF ,SPEC_BUFF_LEN);
...
/*  wyłączone bo waterfall pracuje tylko razem ze spectrum a tam juz dokonano update'u
            if(ts.dial_moved)
            {
                ts.dial_moved = 0;	// Dial moved - reset indicator
...
*/
            ...
            arm_scale_f32(sd.FFT_AVGDataWF, filt_factor, sd.FFT_SamplesWF, SPEC_BUFF_LEN);	// get scaled version of previous data
            arm_sub_f32(sd.FFT_AVGDataWF, sd.FFT_SamplesWF, sd.FFT_AVGDataWF, SPEC_BUFF_LEN);	// subtract scaled information from old, average data
            arm_scale_f32(sd.FFT_MagDataWF, filt_factor, sd.FFT_SamplesWF, SPEC_BUFF_LEN);	// get scaled version of new, input data
            arm_add_f32(sd.FFT_SamplesWF, sd.FFT_AVGDataWF, sd.FFT_AVGDataWF, SPEC_BUFF_LEN);	// add portion new, input data into average

            for(i = 0; i < SPEC_BUFF_LEN; i++)	 		// guarantee that the result will always be >= 0
            {
                if(sd.FFT_AVGDataWF[i] < 1)
                {
                    sd.FFT_AVGDataWF[i] = 1;
                }
            }

            UiSpectrum_CalculateDBm(sd.FFT_SamplesWF, sd.FFT_MagDataWF);
        ...
        case 4:
        {
            ...
            for(i = 0; i < (SPEC_BUFF_LEN); i++)
            {
                sig = log10f_fast(sd.FFT_AVGDataWF[i]) * DB_SCALING_10;		// take FFT data, do a log10 and multiply it to scale 10dB (fixed)
                sig += sd.display_offsetWF;							// apply "AGC", vertical "sliding" offset (or brightness for waterfall)
                if (sig < min1) min1 = sig;
                if(sig > 1)											// is the value greater than 1?
                    sd.FFT_DspDataWF[i] = (q15_t)sig;					// it was a useful value - save it
                else
                    sd.FFT_DspDataWF[i] = 1;							// not greater than 1 - assign it to a base value of 1 for sanity's sake
            }
            //
            ...
            for(i = 0; i < (SPEC_BUFF_LEN); i++)
            {
                if(i < (SPEC_BUFF_LEN/2))	 		// build left half of spectrum data
                {
                    ...
                    sd.FFT_SamplesWF[SPEC_BUFF_LEN - i - 1] = sd.FFT_DspDataWF[i + SPEC_BUFF_LEN/2];	// get data
                }
            ...
            //
            sd.display_offsetWF -= sd.agc_rate*min1/5;
        ...
        case 5:	// rescale waterfall horizontally, apply brightness/contrast, process pallate and put vertical line on screen, if enabled.
        {
            ...
            arm_scale_f32(sd.FFT_SamplesWF, sd.wfall_contrast, sd.FFT_SamplesWF, SPEC_BUFF_LEN);
            ...
            {
                if(sd.FFT_SamplesWF[i] >= NUMBER_WATERFALL_COLOURS)	// is there an illegal color value?
                {
                    sd.FFT_SamplesWF[i] = NUMBER_WATERFALL_COLOURS - 1;	// yes - clip it
                }

                sd.waterfall[sd.wfall_line][i] = (ushort)sd.FFT_SamplesWF[i];	// save the manipulated value in the circular waterfall buffer
            }
            ...
            if(sd.wfall_line==sd.wfall_size)
                sd.wfall_line=0;
        ...
        case 6:
        {
            uchar lptr = sd.wfall_line-1;		// get current line of "bottom" of waterfall in circular buffer
            if(lptr==0xff)
                lptr=sd.wfall_size-1;
            ...
            //if(!sd.wfall_line_update)	 							// if it's count is zero, it's time to move the waterfall up
            {
                ...
                //MarkerON
                uint16_t pixel;
                uint8_t LineDoublecntr=0;

                for(uint16_t lcnt = 0;lcnt < sd.wfall_height; lcnt++)	 				// set up counter for number of lines defining height of waterfall
                {

                    for(i = 0; i < (SPECTRUM_WIDTH); i++)	 	// scan to copy one line of spectral data - "unroll" to optimize for ARM processor
                    {

                        pixel=sd.waterfall_colours[sd.waterfall[lptr][i]];  // write to memory using waterfall color from palette
                        pixel = (pixel >> 8) | (pixel << 8);
                        waterfall_pixel_buf[waterfall_pixel_buf_idx][i] = pixel;

                    }

                    UiLcdHy28_OpenBulkWrite(SPECTRUM_START_X, SPECTRUM_WIDTH, (sd.wfall_ystart + 1)+lcnt, 1);
                    UiLcdHy28_SpiDmaStart((uint8_t*)(&waterfall_pixel_buf[waterfall_pixel_buf_idx][0]),SPECTRUM_WIDTH*2);


                    waterfall_pixel_buf_idx++;
                    waterfall_pixel_buf_idx&=1;
                    ...
                    if(LineDoublecntr==2)
                    {
                        ...
                        if(lptr==0xff)
                            lptr = sd.wfall_size-1;				// clip to display height
                    }
                }
                //MarkerOFF
            ...
            sd.wfal_state = 0;	//wfall state to idle
            //sd.state = 0;   //start collecting data by the audio driver
            break;
        }
        default:
            sd.wfal_state = 0;
...
//
//*----------------------------------------------------------------------------
//* Function Name       : UiSpectrum_DisplayFilterBW
...
void UiSpectrum_DisplayFilterBW()
{
    ...
    {// bail out if in menu mode
        ...
        {
            lpos = FILT_DISPLAY_WIDTH/2+2-(AudioDriver_GetTranslateFreq()/(IQ_SAMPLE_RATE/FILT_DISPLAY_WIDTH));
        }
        else        // magnify mode is on
        {
            lpos = FILT_DISPLAY_WIDTH/2+2;                             // line is alway in center in "magnify" mode
        }
        ...
        UiLcdHy28_DrawStraightLineDouble((POS_SPECTRUM_IND_X), (POS_SPECTRUM_IND_Y + POS_SPECTRUM_FILTER_WIDTH_BAR_Y), POS_SPECTRUM_IND_W, LCD_DIR_HORIZONTAL, Black);
        //UiLcdHy28_DrawStraightLine((POS_SPECTRUM_IND_X), (POS_SPECTRUM_IND_Y + POS_SPECTRUM_FILTER_WIDTH_BAR_Y), POS_SPECTRUM_IND_W, LCD_DIR_HORIZONTAL, Black);
        ...
        if(lpos + width > POS_SPECTRUM_IND_W)                                      // prevents line to leave right border
        {
            width = POS_SPECTRUM_IND_W - lpos;
        }
...
void UiSpectrum_CalcDisplayFilterBW(void)
{
    ...
    uint32_t clr;
        ...
        sd.Pos_FilterStart=POS_SPECTRUM_IND_X+lpos;
        sd.Pos_FilterEnd=sd.Pos_FilterStart+(int16_t)width;
...
static void UiSpectrum_FrequencyBarText()
{
...

    {
       ...
       /* const static int centerIdx2pos[] = {
                POS_SPECTRUM_VGridSpacing*1,
                POS_SPECTRUM_VGridSpacing
                ,94,130,160,192};*/
        ...
        i = (centerIdx+4)*POS_SPECTRUM_VGridSpacing -((strlen(txt)-2)*4);    // calculate position of center frequency text
        uint16_t AddPosY=0;
        if(ts.spectrum_size==SPECTRUM_NORMAL)
            AddPosY=POS_SPECTRUM_INFO_BAR_H;
        UiLcdHy28_PrintText((POS_SPECTRUM_IND_X + i),(POS_SPECTRUM_IND_Y + POS_SPECTRUM_FREQ_BAR_Y + AddPosY),txt,clr,Black,4);


        for (int idx = -3; idx < 4; idx++)
        {
            int pos = POS_SPECTRUM_VGridSpacing*(idx+4);
            if (idx != centerIdx)
            {
                if(sd.magnify < 3)
                {
...
static void UiSpectrum_DisplayDbm()
{
    ...
    if( ts.txrx_mode == TRX_MODE_RX)
    {
        ...
        if ((display_something == true) && (val!=oldVal))
        {
            ...
            UiLcdHy28_PrintText(DisplayDbm_X,DisplayDbm_Y,txt,White,Blue,0);
            UiLcdHy28_PrintText(DisplayDbm_X+SMALL_FONT_WIDTH * 4,DisplayDbm_Y,unit_label,White,Blue,4);
            oldVal=val;
            dBmShown=1;
        }
    }
    ...
    {
        UiLcdHy28_DrawFullRect(DisplayDbm_X, DisplayDbm_Y, 15, SMALL_FONT_WIDTH * 10 , Black);
        dBmShown=0;
        oldVal=99999;
    }
}
    ...
    //
    {
        if( ts.txrx_mode == TRX_MODE_RX && ((ts.s_meter != DISPLAY_S_METER_STD) || (ts.display_dbm != DISPLAY_S_METER_STD )))
        {
            ...
            for(int32_t i = 0; i < (buff_len_int/2); i++)
            {
                ...
                {
                    samplesData[SPEC_BUFF_LEN - i - 1] = MagData[i + buff_len_int/4] * SCOPE_PREAMP_GAIN;	// get data
                }
            ...
            {
                sum_db = sum_db + samplesData[c]; // / (float32_t)(1<<sd.magnify);
            }
